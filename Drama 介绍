...
释放长尾创作者的创作热情与才华，以社区化的方式促进不同创作者的互补以及群体智慧，构建属于社区众创的剧本，并将作品权益释放给社区。初期以剧本作为创作形式，后期逐渐扩展到其它适用于异步创作的品类。并利用Dfinity链上存储的功能逐步发展为艺术领域的去中心化Github。

具体实现方式如下：
构建一个剧本创作平台Drama,其平台币DRM为同质化代币，用户可以在前端网页上使用internet identity登陆：

+ 输入剧本类型、大致梗概、代币总量、众筹时间等信息，点击“剧本申请”按钮，销毁少量DRM作为执行操作的费用，以应对反向Gas机制可能出现的垃圾信息过多的问题，合约自动开始众筹DRM作为剧本社区基金（剧本社区基金可以用来奖励剧本创作者）并创建剧本代币，众筹代币的5%进入Drama平台金库，剩下95%进入剧本金库。用户之后若对该剧本不满意，可通过智能合约用剧本代币按比例赎回众筹时的DRM。**从而为剧本代币构建一个价值下限**。
